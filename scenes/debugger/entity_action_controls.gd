...
func _ready() -> void:
	...
	thunder_button.pressed.connect(_on_thunder_pressed)
...
func _refresh_entity_options() -> void:
	...
	thunder_button.disabled = not has_entity
...
func _thunder_selected_entity() -> void:
	if _selected_entity_id == &"":
		push_warning("Select an entity before casting.")
		return
	...
	_stack_action(_create_cast_action(_selected_entity_id, {
		...
		"value": SUMMON_THUNDER_DAMAGE,
		"source": "debugger",
	}))


func _create_cast_action(entity_id: StringName, args: Dictionary) -> Dictionary:
	return {
